...
function AddBlog() {
  ...
  async function createBlog(values) {
    try {
      ...
      let postData = await axios.post(
        "https://panelserver.digitalkrantiindia.com/api/blog/create",
        data,
        config
      );
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      console.log(values);
      let result = await createBlog(values);
      if (result) {
                ...
                {cardPublish && (
                  <div className="card-body mb-0">
                    ...
                    <div className="form-group">
                      <div className="form-group">
                        <FormControl sx={{ width: "100%" }}>
                          <LocalizationProvider dateAdapter={AdapterDayjs} sx={{height: '20px'}}>
                            <DemoContainer components={["DatePicker"]} >
                              <div className="date">
                                <DesktopDatePicker
                                  sx={{ minWidth: "100%" }}
                                  label="Published on:"
                                  // value={formik.values.publish.publishedOn}
                                  
                                  onChange={(date) => {
                                    // Ensure that 'date' is a valid Dayjs object
                                    if (dayjs.isDayjs(date)) {
                                      const formattedDate =
                                        date.format("YYYY-MM-DD"); // Use dayjs formatting
                  ...
                  <div className="card-body ">
                    <div className="form-group">
                      <img
                        ...
                        src={image}
                        // "https://demo.w3cms.in/cryptozone/public/images/noimage.jpg"
                        alt=""
                      />
