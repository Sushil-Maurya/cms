...
function ProductSubCategory() {
  ...
  async function updateStatus(values) {
    ...
    try {
      const postData = await axios.post(
        "http://localhost:3004/api/product/subCategory/status",
        val,
        config
      );
  ...
  async function handleStatusChange(i) {
    ...
    let values = {
      // updatedby: null,
      subCategoryID: updatedList[i]._id,
  ...
  function getModule(i) {
    setUpdateData((prevData) => ({
      ...prevData,
      categoryID: categoryList[i].categoryID._id,
      subCategoryName: categoryList[i].subCategoryName,
  ...
  const handleDelete = async (e) => {
    const value = { subCategoryID: e.currentTarget.id };
  ...
  const getList = async () => {
    try {
      const postdata = await axios.get(
        "http://localhost:3004/api/product/category/list"
      );
  ...
  async function getCategoryList() {
    ...
    const postData = await axios.post(
      "http://localhost:3004/api/product/subCategory/",
      value,
      config
    );
    ...
    try {
      if (moduleUpdate) {
        ...
        const response = await axios.post(
          "http://localhost:3004/api/product/subCategory/update",
          postvalue,
          config
        );
        return response.data;
        ...
        const response = await axios.post(
          "http://localhost:3004/api/product/subCategory/create",
          val,
          config
        );
        return response.data;
  ...
  useEffect(() => {
    formik.setValues({
      categoryID: moduleUpdate ? updateData.categoryID : "",
      subCategoryName: moduleUpdate ? updateData.subCategoryName : "",
    ...
    <div className="f1">
              ...
              </thead>
              {
                <tbody>
                  {categoryList.map((arr, i) => (
                    <tr key={arr._id}>
                      <td>
                        {arr.categoryID.categoryName
                          ? arr.categoryID.categoryName
                          : ""}
                      </td>
                      <td>{arr.subCategoryName}</td>
                      <td>{arr.slug}</td>
      ...
      >
        <form onSubmit={formik.handleSubmit}>
          <DialogTitle>{"SubCategory Create"}</DialogTitle>
          <DialogContent>
