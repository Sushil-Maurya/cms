...
function Color() {
  ...
  async function updateStatus(values) {
    ...
    try {
      const postData = await axios.post(
        "https://panelserver.digitalkrantiindia.com/api/color/status",
        val,
        config
      );
  ...
  //       "https://panelserver.digitalkrantiindia.com/api/color/list"
  //     );
  ...
  async function getColorList() {
    ...
    const postData = await axios.post(
      "https://panelserver.digitalkrantiindia.com/api/color/",
      value,
      config
    );
    console.log(postData.data.response.docs);
    setColorList(postData.data.response.docs);
    setTotalPage(postData.data.response.totalPages);
  }

  async function UpdateCreateColor(values) {
    try {
      if (moduleUpdate) {
        const postvalue = {
          ...values,
        ...
        const response = await axios.post(
          "https://panelserver.digitalkrantiindia.com/api/color/update",
          postvalue,
          config
        );
        ...
        console.log(val, "333");
        const response = await axios.post(
          "https://panelserver.digitalkrantiindia.com/api/color/create",
          val,
          config
        );
  ...
  async function deleteData(value) {
    try {
      let response = await axios.post(
        "https://panelserver.digitalkrantiindia.com/api/color/delete",
        value,
        config
      );
      // console.log(response.data);
  ...
  function getModule(i) {
    setUpdateData((prevData) => ({
      ...
      colorCode: colorList[i].colorCode,
    }));
    setItemID(colorList[i]._id);
  }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        const result = await UpdateCreateColor(values);
  ...
  useEffect(() => {
    formik.setValues({
      colorName: moduleUpdate ? updateData.colorName : "",
      colorCode: moduleUpdate ? updateData.colorCode : "",
    });
    ...
    <>
      <div className="card">
        <div className="card-header d-flex justify-content-between align-items-center">
          <h4 className="card-title m-2">Colors</h4>
      ...
      >
        <form onSubmit={formik.handleSubmit}>
          ...
          <DialogContent>
            ...
            <div style={{ paddingTop: "10px" }}>
              <FormControl sx={{ width: "400px" }}>
                <InputLabel htmlFor="component-outlined">Color Code</InputLabel>
                <OutlinedInput
                  id="component-outlined"
                  label="Color code"
                  name="colorCode"
